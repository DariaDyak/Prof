...
import { Phone, Mail, CheckCircle, FileText, Users, RefreshCw, Shield, Building, FileCheck, Settings, Lock, Database } from "lucide-react";
import { useState } from "react";

export default function ProfitMoInfo() {
  const [downloading, setDownloading] = useState<string | null>(null);

  const doc1Url = "/attached_assets/generated_images/Описание_Profit-MO.pdf";
  const doc2Url = "/attached_assets/generated_images/Руководство_Profit-MO.pdf";
  ...
  const productInfo = {
    ...
    purpose: "Автоматизация обмена данными и элементами управления между системой ЭСМО и 1С, создание среды для сбора, обмена, хранения и обработки информации о медицинских осмотрах.",

    problems: [
      "Нет инструментов взаимодействия с ЭСМО из 1С:Бухгалтерия",
      "Нет возможности партнерам Квазар получать данные через 1С о медосмотрах по каждому Клиенту системы ЭСМО для формирования бухгалтерской документации",
      "Высокие затраты человеческих ресурсов на ведение учета в двух системах",
      "Ручная обработка информации повышает риски генерации ошибок в 1С:Бухгалтерия"
    ],

    benefits: [
      "Уменьшение трудозатрат на сбор данных из систем за счет автоматизации процесса",
      "Предотвращение ошибок ввода данных благодаря автоматизированному контролю входной информации",
      "Управление всем циклом взаимодействия с ЭСМО из 1С:Бухгалтерия",
      "Автоматическое формирование счетов и закрывающих документов за медосмотры по тарифной сетке Клиента"
    ],
    ...
    modules: [
      {
        ...
        description: "Работа со списком клиентов, групп и организаций. Просмотр всех контрагентов, структуры организаций, добавление новых клиентов, получение данных по прошедшим медосмотрам и формирование счетов на оплату."
      },
      {
      ...
      {
        name: "Автоматическое формирование счетов",
        icon: <RefreshCw className="w-4 h-4" />,
        description: "Автоматическое формирование счетов на оплату и закрывающих документов на основании условий договора и количества проведенных медосмотров. Поддержка авансирования и постоплаты."
      },
      {
        name: "Гибкие схемы выставления счетов",
        icon: <Settings className="w-4 h-4" />,
        description: "Поддержка нестандартных схем: счета могут формироваться сводно по всем организациям или по каждой организации отдельно."
      },
      {
    ...
    technologies: {
      ...
      integration: "Интеграция с 1С:Бухгалтерия и системой ЭСМО",
      features: [
        "Двусторонний обмен данными",
        "Автоматическая синхронизация",
        "Веб-сервисы для интеграции",
    ...
    documentation: [
      {
        title: "Техническое описание и руководство по интеграции",
        url: doc1Url,
        filename: "Техническое_описание_Profit-MO.pdf"
      },
      {
        title: "Руководство пользователя и администратора",
        url: doc2Url,
        filename: "Руководство_пользователя_Profit-MO.pdf"
      }
    ],
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
              ...
              <div className="p-4 sm:p-6 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-800 dark:to-blue-900/30 rounded-xl sm:rounded-2xl border border-blue-100 dark:border-blue-800/50">
                ...
                <p className="text-foreground/90 text-sm sm:text-base mb-3 sm:mb-4 text-justify">
                  <span className="font-medium">Интеграция:</span> {productInfo.technologies.integration}
                </p>
            ...
            <div className="mb-6 sm:mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
                <div className="p-3 sm:p-4 bg-white dark:bg-transparent rounded-lg sm:rounded-xl border border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-600 dark:shadow-none shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden flex flex-col">
                  ...
                  <p className="text-xs sm:text-sm text-foreground/80 text-justify">Полная автоматизация взаимодействия между ЭСМО и 1С</p>
                </div>
